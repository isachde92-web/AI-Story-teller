...
with st.form("story_form"):
    ...
    length = st.selectbox(
        "Length",
        ["Very short (2-4 paragraphs)", "Short (6-8 paragraphs)", "Medium (10-12 paragraphs)"],
        index=1
    )
...
def build_prompt(name, theme, age_group, tone, length, include_moral):
    ...
    prompt += (
        "Do NOT include any adult content, frightening descriptions, or complex vocabulary. "
    ...
    if not api_key:
        ...
        if story:
            ...
            with st.expander("Create a 1-minute reading (shortened)"):
                try:
                    mini_prompt = (
                        f"Shorten the following story to about 150 words, keeping the tone, characters, "
                        f"and moral intact:\n\n{story}"
                    )
